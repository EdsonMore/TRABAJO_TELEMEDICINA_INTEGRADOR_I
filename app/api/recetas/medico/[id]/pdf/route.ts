// app/api/recetas/medico/[id]/pdf/route.ts - VERSIÓN COMPLETAMENTE CORREGIDA
...
export async function GET(
  ...
  let client;
  try {
    // ✅ CORRECCIÓN: Validación robusta del token
    ...
    // ✅ Solo médicos pueden generar PDFs de recetas
    if (usuario.rol !== "medico") {
      return NextResponse.json(
        { error: "Solo los médicos pueden generar recetas" },
        { status: 403 }
      );
    }
    ...
    // ✅ CORRECCIÓN: Verificar que el médico tiene permisos sobre esta receta
    const result = await client.query(
      ...
      SELECT 
        ...
        -- Verificar que el médico actual es el dueño de la receta
        CASE WHEN m.id_usuario = $2 THEN true ELSE false END as tiene_permiso,
        -- Medicamentos
        COALESCE(
          json_agg(
            json_build_object(
      ...
      `,
      [id, usuario.userId] // ✅ Agregar userId para verificación de permisos
    );
    ...
    // ✅ CORRECCIÓN: Verificar permisos del médico
    if (!receta.tiene_permiso) {
      return NextResponse.json(
