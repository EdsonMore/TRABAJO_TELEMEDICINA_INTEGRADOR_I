...
export const useWebRTC = (roomId, userData) => {
    ...
    // Conectar al WebSocket - CORREGIDO
    const connectWebSocket = useCallback(() => {
        ...
        try {
            ...
            const port = process.env.NODE_ENV === 'development' ? '3002' : window.location.port;
            ...
            websocket.onopen = () => {
                ...
                setTimeout(() => {
                    if (websocket.readyState === WebSocket.OPEN) {
                        ...
                        console.log('📨 Mensaje join-room enviado para room:', roomId);
                    }
                }, 500);
            };
            ...
            websocket.onmessage = (event) => {
                try {
                    ...
                    console.log('📩 Mensaje WebSocket recibido:', data.type, 'de:', data.from);
            ...
            websocket.onclose = (event) => {
                ...
                // Intentar reconectar solo si es apropiado
                if (shouldReconnect(event)) {
                    ...
                    console.error('❌ Máximo de intentos de reconexión alcanzado o cierre intencional');
                    setConnectionStatus('error');
                }
            };

            // CORREGIDO: Manejo mejorado de errores de WebSocket
            websocket.onerror = (event) => {
                console.error('❌ WebSocket error event:', event);
                // En la mayoría de navegadores, el evento error no tiene detalles específicos
    ...
    const handleSignalingMessage = useCallback(async (data) => {
        ...
        try {
            switch (data.type) {
                case 'joined-room':
                    console.log('✅ Unido a la sala. Participantes:', data.participants);
                    const otherParticipants = data.participants.filter(p => p.userId !== userData.id);
                ...
                case 'chat-message':
                    console.log('💬 Chat message recibido:', data.message, 'de:', data.from);
                ...
                case 'media-status':
                    console.log('📊 Estado de medios actualizado:', data.userId, data.mediaType, data.enabled);
    ...
    const createPeerConnection = useCallback(async (targetUserId, isInitiator) => {
        try {
            console.log(`🔗 Creando PeerConnection con ${targetUserId}, iniciador: ${isInitiator}`);
            ...
            if (localStreamRef.current) {
                console.log(`🎥 Añadiendo stream local a conexión con ${targetUserId}`);
                localStreamRef.current.getTracks().forEach(track => {
                    try {
            ...
            peerConnection.ontrack = (event) => {
                console.log(`🎥 Track remoto recibido de ${targetUserId}:`, event.track.kind, event.streams);
                ...
                if (event.streams && event.streams.length > 0) {
                    ...
                    if (activeTracks.length === 0) {
                        console.warn(`⚠️ Stream de ${targetUserId} no tiene tracks activos`);
                        return;
                    }

                    // Escuchar cambios en los tracks
                    event.track.onended = () => {
                        console.log(`⏹️ Track ${event.track.kind} de ${targetUserId} terminó`);
                    };

                    event.track.onmute = () => {
                        console.log(`🔇 Track ${event.track.kind} de ${targetUserId} muteado`);
                    };
                    ...
                    setRemoteStreams(prev => {
                        ...
                        newStreams.set(targetUserId, {
                            stream: remoteStream,
                            userData: existingData?.userData || participants.find(p => p.userId === targetUserId)?.userData
                        });
                        console.log(`✅ Stream remoto actualizado para ${targetUserId}, tracks activos:`, activeTracks.length);
                        return newStreams;
                    });
                }
            };
            ...
            peerConnection.onconnectionstatechange = () => {
                ...
                if (state === 'connected') {
                    ...
                    console.log(`❌ Conexión ${state} con ${targetUserId}, manejando...`);
            ...
            peerConnection.onsignalingstatechange = () => {
                console.log(`📡 Signaling state con ${targetUserId}:`, peerConnection.signalingState);
            };
            ...
            if (isInitiator) {
                try {
                    ...
                    console.error(`❌ Error creando oferta para ${targetUserId}:`, error);
                    addError(error);
                }
            }
            ...
            console.error(`❌ Error creando peer connection con ${targetUserId}:`, error);
            addError(error);
            throw error;
        }
    ...
    const handleOffer = useCallback(async (fromUserId, offer) => {
        try {
            ...
            if (!peerConnection) {
                console.log(`🔗 Creando nueva conexión para respuesta de ${fromUserId}`);
    ...
    const initializeLocalStream = useCallback(async () => {
        try {
            ...
            console.log('✅ Stream local obtenido:', stream.getTracks().map(t => `${t.kind}-${t.id}`));
            ...
            peerConnections.current.forEach((pc, userId) => {
                ...
                stream.getTracks().forEach(track => {
                    ...
                    if (sender) {
                        console.log(`🔄 Reemplazando ${track.kind} track en conexión con ${userId}`);
                        sender.replaceTrack(track).catch(err =>
                            console.error('Error reemplazando track:', err)
                        );
                    } else {
                        console.log(`➕ Añadiendo ${track.kind} track en conexión con ${userId}`);
    ...
    const toggleMedia = useCallback((mediaType, enabled) => {
        try {
            if (localStreamRef.current) {
                const tracks = mediaType === 'audio'
                    ? localStreamRef.current.getAudioTracks()
                    : localStreamRef.current.getVideoTracks();

                console.log(`🔊 Toggling ${mediaType}: enabled=${enabled}, tracks found: ${tracks.length}`);

                tracks.forEach(track => {
                    track.enabled = enabled;
                    console.log(`✅ ${mediaType} track (${track.kind}): ${enabled ? 'ACTIVADO' : 'DESACTIVADO'}`);
                });
