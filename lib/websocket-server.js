...
class WebRTCSignalingServer {
  constructor(server) {
    // ✅ Recibir el servidor HTTP
    this.wss = new WebSocketServer({ server }); // ✅ Usar mismo servidor
    this.rooms = new Map();
    this.setupWebSocket();
    console.log(
      `📡 WebSocket Signaling Server ejecutándose en el mismo puerto HTTP`
    );
  }
  ...
  handleMessage(ws, data) {
    ...
    console.log(
      `📨 Mensaje tipo: ${type}, Sala: ${roomId}, Usuario: ${userId}`
    );
    ...
    switch (type) {
      ...
      case "chat-message":
        ...
        console.log(
          `💬 Mensaje de chat de ${userId} enviado a otros usuarios en sala ${roomId}`
        );
        break;
      ...
      case "media-status":
        ...
        console.log(
          `🔊 Estado de media actualizado: ${payload.mediaType} ${
      ...
      case "end-session":
        // 🔥 MÉDICO FINALIZANDO SESIÓN - Notificar a TODOS en la sala
        console.log(`🛑 Sesión finalizada por médico en sala ${roomId}`);
        this.broadcastToRoom(roomId, null, {
          type: "session-ended",
          endedBy: userId,
          message: "El médico ha finalizado la consulta",
          timestamp: new Date().toISOString(),
        });
        break;
  ...
  joinRoom(ws, roomId, userId, userData) {
    ...
    console.log(
      `👤 Usuario ${userId} se unió a sala ${roomId}. Participantes: ${room.size}`
    );
  }
  ...
  relayMessage(roomId, fromUserId, targetUserId, type, payload) {
    ...
    if (targetUserId) {
      ...
      if (targetUser && targetUser.ws.readyState === WebSocket.OPEN) {
        ...
        console.warn(
          `Usuario objetivo ${targetUserId} no encontrado o desconectado`
        );
      }
    } else {
  ...
  broadcastToRoom(roomId, excludeUserId, message) {
    ...
    console.log(
      `📢 Broadcast enviado a ${sentCount} usuarios en sala ${roomId}`
    );
  }
